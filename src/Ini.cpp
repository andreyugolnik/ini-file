...
unsigned cFile::write(void* buffer, unsigned size)
{
...
cSection* cIni::parseLine(const std::string& s, cSection* currentSection)
{
    auto len = s.length();
    if (len > 2 && s[0] == '[' && s[len - 1] == ']')
    {
        auto name = s.substr(1, s.length() - 2);
        currentSection = find(name.c_str());
...
void cIni::read(cFile* file)
{
    ...
    for (auto& line : lines)
    {
        currentSection = parseLine(line, currentSection);
    }
}
